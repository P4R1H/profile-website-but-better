...
const PerspectiveAccordion = ({
  ...
  useEffect(() => {
    if (!isMobile) {
      // Switching to desktop - open Product or first available if nothing is selected
      if (activeId === null) {
  ...
  const handleAccordionToggle = (perspectiveId: PerspectiveType, isCurrentlyActive: boolean) => {
    ...
    if (isMobile && newActiveId && scrollContainerRef?.current) {
      ...
      if (accordionEl) {
        // Wait for the accordion animation to mostly complete
        scrollTimeoutRef.current = setTimeout(() => {
          ...
          // Calculate target position: align top of accordion with top of container + offset
        ...
        }, 300); // 300ms delay to allow layout shift to stabilize
      }
    }
  };
    ...
    return (
      <div className="w-full h-full flex flex-col">
        ...
        <div className="shrink-0 flex border-b border-zinc-800">
          {perspectives.map((perspective) => (
            <button
              ...
              className={`relative px-6 py-3 text-sm font-medium transition-colors ${activeId === perspective.id
                  ? "text-zinc-100"
        ...
        <div className="flex-1 min-h-0 relative">
          ...
          {/* Bottom Blur */}
          <div
            className={`absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none transition-opacity duration-300 ${showBottomBlur ? "opacity-100" : "opacity-0"
              }`}
          />
        </div>
      </div>
  ...
  return (
    <div className="w-full space-y-2 pb-8">
      {perspectives.map((perspective) => {
        ...
        return (
          <motion.div
            layout // Smoothly animate position changes when siblings expand/collapse
            key={perspective.id}
            ref={(el) => {
          ...
          >
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 transition-colors ${isActive ? "bg-zinc-900/50" : "hover:bg-zinc-900/30"
                }`}
            >
              <span className={`text-sm font-medium transition-colors ${isActive ? "text-zinc-100" : "text-zinc-400"
                }`}>
                {perspective.label}
              </span>
...
export const ProjectExpanded = ({ project, onClose }: ProjectExpandedProps) => {
  ...
  return (
    <div className="w-full h-full bg-black relative overflow-hidden">
      ...
      <div className="absolute -top-2 left-0 right-0 h-8 md:h-12 md:top-0 bg-gradient-to-b from-black to-transparent z-10 pointer-events-none" />
      ...
      <div className={`md:hidden absolute -bottom-8 left-0 right-0 h-24 bg-gradient-to-t from-black via-black/80 to-transparent z-10 pointer-events-none transition-opacity duration-300 ${showBottomBlur ? 'opacity-100' : 'opacity-0'
        }`} />
    </div>
  );
};
